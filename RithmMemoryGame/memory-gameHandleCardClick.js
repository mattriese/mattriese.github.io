...
function createCards(colors) {
  ...
  let totalMatched = gameBoard.dataset.totalMatched;
  totalMatched = 0;
  let temp;
  ...
  for (let color of colors) {
    ...
    card.addEventListener('click', function(e){
        if(e.target.dataset.matched === true || e.target === temp){
            return;
          }
          flipCard(e.target);

          // if this is the first of a potential pair to be clicked, set the temp variable to this card
          if(temp === undefined){
            temp = e.target;
          }
          // if this is the second of a potential pair to be clicked, compare their colors
          else if(e.target.classList[1] === temp.classList[1]){ //if their colors are the same:
            e.target.dataset.matched = true;
            temp.dataset.matched = true;
            gameBoard.dataset.totalMatched += 2;
          }
          else{ //if their colors are different
            setTimeout(unFlipCard, 1000, e.target, temp);
            temp = undefined;
          }
    });
...
function flipCard(card) {
  card.style.backgroundColor = `${e.target.classList[1]}`;
}
